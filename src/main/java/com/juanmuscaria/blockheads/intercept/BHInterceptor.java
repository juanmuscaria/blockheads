...
public class BHInterceptor {
  ...
  public static void main(String... args) {
    ...
    try (Arena allocator = Arena.ofConfined()) {
      ...
      ENetAddress.host$set(proxyHost, ENet.ENET_HOST_ANY());

      var serverHost = ENetAddress.allocate(allocator);
      ENetAddress.port$set(serverHost, SERVER_PORT);
      ENet.enet_address_set_host(serverHost, allocator.allocateUtf8String(SERVER_HOST));

      var proxyServer = ENet.enet_host_create(proxyHost, 1, CHANNELS, 0, 0);
      ...
      while (true) {
        ...
        while (ENet.enet_host_service(proxyServer, event, 10) != 0) {
          ...
          if (type == ENet.ENET_EVENT_TYPE_CONNECT()) {
            ...
            realServerPeer = ENet.enet_host_connect(realServer, serverHost, 2, 0);
